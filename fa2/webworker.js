...
moduleShim.exports = function iterate(options, NodeMatrix, EdgeMatrix) {
  ...
  if (adjustSizes === true) {
    ...
    for (n = 0; n < order; n += PPN) {
      if (true) {
        ...
        NodeMatrix[n + NODE_CONVERGENCE] = 1;
  ...
  // CUSTOM CODE HERE:
  // Restrict each node on its circle
  var d, radius;

  for (n = 0; n < order; n += PPN) {
    newX = NodeMatrix[n + NODE_X];
    newY = NodeMatrix[n + NODE_Y];
    radius = NodeMatrix[n + NODE_CIRCLE_RADIUS];

    d = Math.sqrt(newX * newX + newY * newY);

    // UNCOMMENT THIS CODE TO ALLOW SOME BANDWIDTH AROUND THE CIRCLES
    // (so that nodes can cross):
    // if (d > radius * 1.1) radius = radius * 1.1;
    // else if (d < radius * 0.9) radius = radius * 0.9;
    // else radius = d;

    NodeMatrix[n + NODE_X] = newX * radius / d;
    NodeMatrix[n + NODE_Y] = newY * radius / d;
  }
